...
uses Forms, Windows, StdCtrls, Messages, Classes, JclMath, MahBit32, SysUtils, strutils_,
...
type
  ...
  ScriptFile = record
    ...
    Counter2:          Byte;
    Counter3:          Byte;
    Description:       array [0..39] of Char;
    ScriptFlags:       Integer;
...
uses compileProgress, Math, Dialogs;
...
begin
  ...
  begin
    if src.Count <> dest.Count then
      ConsoleDebug('Line count mismatch - source=' + IntToStr(src.Count) + ' dest=' + IntToStr(dest.Count));
...
begin
  ...
  begin
    ...
    begin
      ...
      if (verbosedebug = False) and (outputnormally = True) then
      begin
        memo_output.Lines.add(thestr);
        Memo_output.Perform(EM_LineSCROLL, 1, memo_output.Lines.Count);
      end else
      if (verbosedebug = True) and ((outputnormally = True) or (outputnormally = False)) then
      begin
        memo_output.Lines.add(thestr);
        Memo_output.Perform(EM_LineSCROLL, 1, memo_output.Lines.Count);
      end;
...
begin
  ...
  begin
    ...
    begin
      install_ok := False;
      MessageDlg('Arcanum installation was not found...' + #13 + #10 + '' + #13 + #10 + 'This will cause the module-editing features' +
        #13 + #10 + 'to be disabled.' + #13 + #10 + '' + #13 + #10 + 'Sorry for the inconvenience.', mtWarning, [mbOK], 0);
      ConsoleDebug('Arcanum not found ... Reverting to ScriptEd-supplied opcodes');
  ...
  begin
    ...
    begin
      consoledebug('--> FAILURE - Arcanum.exe does NOT exist in ' + arcanumpath);
      arcanum_is_installed := False;
    end;
  ...
  begin
    ...
    begin
      opendatfile(actiondathandle, actionopcodesdat, arcanumpath + '\arcanum4.dat');
      openfilefromdat(actiondathandle, actionopcodesdat, 'semes\action.mes', 'action.mes');

    end else
      openfilefromdat(dathandle, opcodesdat, 'semes\action.mes', 'action.mes');
    //   opendatfile(
    openfilefromdat(dathandle, opcodesdat, 'semes\condition.mes', 'condition.mes');
...
function ReverseOpcodeLookup(sourcestring: String; messagedata: messagefile): Integer;
var
  dest: String;
  currentword: String;
  lookahead: String;
...
begin
  ...
  begin
    if (pos(focusdata.entries[t].messagestr, dest) <> 0) and (pos('PC Variable', dest) = 0) and
      (pos('PC Flag', dest) = 0) and (t <> 11) then
      dest := StringReplace(dest, focusdata.entries[t].messagestr, '(obj)', [rfReplaceAll]);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        dest := StringReplace(dest, currentword + ' ' + lookahead + ' ' + extractword(t + 2, dest, [':', ' ', ',']), '(obj)', []);
      end;
      if lookahead = 'Flag' then
      begin
        dest := StringReplace(dest, currentword + ' ' + lookahead + ' ' + extractword(t + 2, dest, [':', ' ', ',']), '(num)', []);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if charcnt > 39 then
        begin
          Consoledebug('WARNING! Script description was truncated because it was too long!');
    ...
    if (input[i] <> '') and (pos('.', input[i]) >= 2) and (pos('.', input[i]) < 6) then
    begin
      line := copy(input[i], pos('.', input[i]) + 2, length(input[i])) + #13#10;
      ...
      begin
        ...
        application.ProcessMessages;
        ParseScriptLine(line, currentscript.scriptlines[linecounter]^, temp_result, linecounter);
    ...
    if (input[i] <> '') and (pos('.', input[i]) = 0) and (i > 2) and (pos('then', lowercase(input[i])) = 0) and
      (pos('else', lowercase(input[i])) = 0) and (pos('if', lowercase(input[i])) = 0) then
    begin
      ...
      new(currentscript.scriptlines[linecounter]);
      application.ProcessMessages;
      ParseScriptLine(input[i], currentscript.scriptlines[linecounter]^, temp_result, linecounter);
      //  form1.linescompiledpanel.Caption := 'Lines compiled: ' + IntToStr(linecounter);
      if temp_result = False then
      begin
        Inc(failedlines);
    ...
    if (input[i] <> '') and (pos('.', input[i]) = 0) and (i > 2) and ((pos('then', lowercase(input[i])) <> 0) or
      (pos('else', lowercase(input[i])) <> 0) or (pos('if', lowercase(input[i])) <> 0)) then
    begin
      line := line + input[i] + #13#10;

      if (pos('then', lowercase(line)) <> 0) and (pos('else', lowercase(input[i + 1])) = 0) then
      begin
        setlength(currentscript.ScriptLines, linecounter + 1);
        new(currentscript.scriptlines[linecounter]);
        application.ProcessMessages;
      ...
      if (pos('then', lowercase(line)) <> 0) and (pos('else', lowercase(line)) <> 0) then
      begin
  ...
  if (currentscript.LineCount > 10) and (currentscript.LineCount div (currentscript.LineCount div 10) = 10) then
  begin
    // consoledebug('divisible with ten. rounding with '+inttostr(currentscript.LineCount)+'+4 = '+inttostr(currentscript.LineCount+4));
    currentscript.MaxLinesAllocated := trunc(SimpleRoundTo(currentscript.LineCount + 4, 1));
  end else
  begin
...
procedure ReplaceParamWithData(param: Integer; str: String; var dest: String; Data: String);
var
  t: Integer;
  curword: String;
  tempcount: Integer;
...
begin
  ...
  begin
    ...
    if (ExtractWord(t, str, [':', ' ', ',']) = '(obj)') or (ExtractWord(t, str, [':', ' ', ',']) = '(num)') then
    begin
      Inc(paramnum);
    end;
    if paramnum = param then
...
begin
  ...
  Result := Result + 'MAX_LINES_ALLOCATED ' + IntToStr(scr.MaxLinesAllocated) + #13#10;
...
begin
  ...
  begin
    ...
    condparams := getopcodeparamcount(getMesstringbyid(scrline.ifPart.opcode, ConditionOpcodes));
    ...
    begin
      ...
      if paramtype = PARAM_TYPE_NUM then
        datastr := getmesstringbyid(scrline.ifpart.VarTypes[t], ValueData);

      if (paramtype = PARAM_TYPE_OBJ) then
        ReplaceParamWithData(t + 1, condlinetemp, condlinefinal, datastr)
      else
      if (paramtype = PARAM_TYPE_NUM) and (scrline.ifpart.VarTypes[t] = 3) then
      begin
        ReplaceParamWithData(t + 1, condlinetemp, condlinefinal, IntToStr(scrline.ifpart.VarValue[t]));
      end else
      begin

        if (scrline.ifpart.VarTypes[t] = 5) or (scrline.ifpart.VarTypes[t] = 6) then
        begin
          datastr2 := GetMesStringByID(hiword(scrline.ifpart.VarValue[t]), FocusData);
          ReplaceParamWithData(t + 1, condlinetemp, condlinefinal, datastr + ' ' + IntToStr(loword(scrline.ifpart.VarValue[t])) +
            ' of ' + datastr2);
        end else

          ReplaceParamWithData(t + 1, condlinetemp, condlinefinal, datastr + ' ' + IntToStr(scrline.ifpart.VarValue[t]));
      end;
  ...
  begin
    thenparams := getopcodeparamcount(getMesstringbyid(scrline.thenPart.opcode, actionOpcodes));
    ...
    begin
      ...
      begin
        if scrline.thenpart.VarTypes[t] = 11 then
          datastr := getmesstringbyid(scrline.thenpart.VarTypes[t], focusData) + ' ' + IntToStr(scrline.thenpart.VarValue[t])
        else
      ...
      if (paramtype = PARAM_TYPE_NUM) and (scrline.thenpart.VarTypes[t] = 3) then
      begin
        ReplaceParamWithData(t + 1, thenlinetemp, thenlinefinal, IntToStr(scrline.thenpart.VarValue[t]));
      end else
      begin
        if (paramtype = PARAM_TYPE_NUM) and (scrline.thenpart.VarTypes[t] <> 3) then
        begin

          if (scrline.thenpart.VarTypes[t] = 5) or (scrline.thenpart.VarTypes[t] = 6) then
          begin
            datastr2 := GetMesStringByID(hiword(scrline.thenpart.VarValue[t]), FocusData);
            ReplaceParamWithData(t + 1, thenlinetemp, thenlinefinal, datastr + ' ' + IntToStr(
              loword(scrline.thenpart.VarValue[t])) + ' of ' + datastr2);
          end else
            ReplaceParamWithData(t + 1, thenlinetemp, thenlinefinal, datastr + ' ' + IntToStr(scrline.thenpart.VarValue[t]));
        end;
  ...
  begin
    elseparams := getopcodeparamcount(getMesstringbyid(scrline.elsePart.opcode, actionOpcodes));
    ...
    begin
      ...
      if (paramtype = PARAM_TYPE_NUM) and (scrline.elsepart.VarTypes[t] = 3) then
      begin
        ReplaceParamWithData(t + 1, elselinetemp, elselinefinal, IntToStr(scrline.elsepart.VarValue[t]));
      end else
      begin
        if (paramtype = PARAM_TYPE_NUM) and (scrline.elsepart.VarTypes[t] <> 3) then
        begin

          if (scrline.elsepart.VarTypes[t] = 5) or (scrline.elsepart.VarTypes[t] = 6) then
          begin
            datastr2 := GetMesStringByID(hiword(scrline.elsepart.VarValue[t]), FocusData);
            ReplaceParamWithData(t + 1, elselinetemp, elselinefinal, datastr + ' ' + IntToStr(
              loword(scrline.elsepart.VarValue[t])) + ' of ' + datastr2);
          end else
            ReplaceParamWithData(t + 1, elselinetemp, elselinefinal, datastr + ' ' + IntToStr(scrline.elsepart.VarValue[t]));
        end;
...
begin
  ...
  begin
    ...
    if form1 <> nil then
      form1.filesizepanel.Caption := 'File Size: ' + IntToStr(filesize(f)) + ' bytes';
  end;
...
begin
  ...
  begin
    scr.script_id := StrToInt(scrid);
    consoledebug('Script ID for ' + extractfilename(filename) + ': ' + IntToStr(scr.script_Id));
  end else
  begin
    scr.script_id := -1;
    consoledebug('No valid script id for ' + extractfilename(filename) + ', set to -1');
  end;
  BlockRead(f, scr.LocalFlags, 4);
  BlockRead(f, scr.Counter0, 1);
  BlockRead(f, scr.Counter1, 1);
