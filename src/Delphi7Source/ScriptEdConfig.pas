...
begin
  ...
  cfg.WriteBool('General', 'ConfirmPlayerOptionDelete', ConfirmPlayerOptionDelete);
  ...
  begin
    cfg.WriteString('Dialogue Editor', 'GenderString' + IntToStr(g + 1) + 'male', genderstrings[g].male);
    cfg.WriteString('Dialogue Editor', 'GenderString' + IntToStr(g + 1) + 'female', genderstrings[g].female);
  end;

  //arcanumpath := extractfilepath(arcanumpath);
  cfg.WriteBool('Dialogue Editor', 'GenderSpecificReplace', AutoReplaceGenderStrings);
...
begin
  ...
  begin
    ...
    genderstrings[g - 1].male := cfg.ReadString('Dialogue Editor', 'GenderString' + IntToStr(g) + 'male', '');
    genderstrings[g - 1].female := cfg.ReadString('Dialogue Editor', 'GenderString' + IntToStr(g) + 'female', '');
    Inc(genderstringcnt);
  end;

  //arcanumpath := extractfilepath(arcanumpath);
  AutoReplaceGenderStrings := cfg.ReadBool('Dialogue Editor', 'GenderSpecificReplace', False);
  ...
  AutoUpdateFemaleLine := cfg.ReadBool('General', 'AutoUpdateFemaleLine', False);
  IncludeEntryPoints := cfg.ReadBool('General', 'IncludeEntryPoints', True);

  ConfirmPlayerOptionDelete := cfg.ReadBool('General', 'ConfirmPlayerOptionDelete', True);
  PlayerOptionCommentEOL := cfg.ReadBool('General', 'PlayerOptionCommentEOL', False);
  ShowCompressedModules := cfg.ReadBool('General', 'ShowCompressedModules', True);
