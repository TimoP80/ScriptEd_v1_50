...
uses
  ...
  JclFileUtils, moduleloader, scriptedconfig, Mesfileio, mesparser, Dialogs, StdCtrls, JvExStdCtrls, JvHtControls;
...
begin
  ...
  advbuildfilelist(arcanumpath + '\Modules\' + modulefolder + '\*.mes', faAnyfile, strs, amAny, [flRecursive, flFullNames]);
  form16.JvHTListBox1.Clear;
  for t := 0 to strs.Count - 1 do
  begin
    linedata := ExtractRelativePath(arcanumpath+'\Modules\'+modulefolder+'\', strs[t]);
