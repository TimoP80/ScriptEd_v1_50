...
type
  ...
  TCrashLogClass = class of TCrashLog;

var
  CrashLogClass: TCrashLogClass = TCrashLog;
...
begin
  if JclValidateModuleAddress(ExceptAddr)
    ...
    TCrashLog.ExceptionHandler(nil, Exception(ExceptObject))
  else
    SysUtils.ShowException(ExceptObject, ExceptAddr);
end;
...
procedure TCrashLog.AfterCreateDetails;
begin
...
procedure TCrashLog.BeforeCreateDetails;
begin
...
function TCrashLog.ReportMaxColumns: Integer;
begin
  Result := 78;
end;
...
procedure TCrashLog.CopyReportToClipboard;
begin
...
procedure TCrashLog.CreateDetails;
begin
...
procedure TCrashLog.CreateReport;
var
  SL: TStringList;
  I: Integer;
...
procedure TCrashLog.DetailsBtnClick(Sender: TObject);
begin
...
class procedure TCrashLog.ExceptionHandler(Sender: TObject; E: Exception);
begin
  if Assigned(E) then
...
class procedure TCrashLog.ExceptionThreadHandler(Thread: TJclDebugThread);
var
  E: Exception;
begin
...
procedure TCrashLog.FormCreate(Sender: TObject);
begin
...
procedure TCrashLog.FormDestroy(Sender: TObject);
begin
...
procedure TCrashLog.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TCrashLog.FormPaint(Sender: TObject);
begin
...
procedure TCrashLog.FormResize(Sender: TObject);
begin
...
procedure TCrashLog.FormShow(Sender: TObject);
begin
  BeforeCreateDetails;
...
function TCrashLog.GetReportAsText: string;
begin
...
procedure TCrashLog.NextDetailBlock;
begin
...
function TCrashLog.ReportNewBlockDelimiterChar: Char;
begin
  Result := '-';
end;
...
procedure TCrashLog.SetDetailsVisible(const Value: Boolean);
const
...
class procedure TCrashLog.ShowException(E: TObject; Thread: TJclDebugThread);
begin
  if CrashLog = nil then
    CrashLog := CrashLogClass.Create(Application);
  try
    with CrashLog do
    begin
...
procedure TCrashLog.UMCreateDetails(var Message: TMessage);
begin
  Update;
  CreateDetails;
end;
...
procedure TCrashLog.UpdateTextMemoScrollbars;
begin
...
begin
  ...
  begin
    ...
    AppEvents.OnException := TCrashLog.ExceptionHandler;
    ...
    JclDebugThreadList.OnSyncException := TCrashLog.ExceptionThreadHandler;
    JclHookThreads;
