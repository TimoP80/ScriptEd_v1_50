...
uses Windows, ScriptEdConfig, SysUtils, arcdatlib, ArcanumSCRLib, MesFileIO, MESParser;
...
var
  ...
  GameRD_npc_f2f, GameRD_npc_f2m, GameRD_npc_m2f, GameRD_npc_m2m, GameRD_npc_m2m_dumb: Messagefile;
  ...
  // written stuff
  GameNewsPaper, GameBook, GameNote, GameDesc, GameKey, GameTelegram, StoryState: MessageFile;
  ...
  AttachmentPoints, AttachmentPointDesc, Stats, ScriptCallTypes, MesFileGeneric: MessageFile;
...
begin
  ...
  if getfileindex(moduleDATHandle, 'oemes\gameoname.mes') <> -1 then
    ...
    GameRD_npc_m2m_dumb := ParseMESFromModuleDAT('mes\game_rd_npc_m2m_dumb.mes');
  ...
  begin
    ...
    begin
      GameOName := ParseMESFromDAT('oemes\oname.mes', arcanum3hnd, arcanumpath + '\arcanum3.dat', arcanum3dat);
    end else
    begin
  ...
  consoledebug(format('Loading module took in %0.7f seconds', [elapsedseconds]));
end;
