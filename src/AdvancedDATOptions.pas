...
uses
  ...
  moduleloader, CompressionSettings, JclFileUtils, ScriptEdConfig, Dialogs,
  StdCtrls, CheckLst, JvExCheckLst, JvCheckListBox,
...
begin
  ...
  AdvBuildFileList(arcanumpath + '\Modules\' + selectedmodule + '\*', faAnyFile,
    filelist, amAny, [flRecursive, flFullNames]);
  for t := 0 to filelist.Count - 1 do
  begin
    foldername := ExtractRelativePath(arcanumpath + '\Modules\' + selectedmodule
      + '\', filelist[t]);
    foldername := extractfiledir(foldername);
    if (folderlist.IndexOf(foldername) = -1) and
      (isdirectory(arcanumpath + '\Modules\' + selectedmodule + '\' +
      foldername)) and (foldername <> '') then
    begin
...
begin
  ...
  for t := 0 to groupcnt - 1 do
  begin
    Form22.grp_list.Items.Add('Group ' + IntToStr(t + 1) + ' => .' +
      split_groups[t].extension);
  end;
...
begin
  ...
  if fileexists(arcanumpath + '\Modules\' + modulefolder +
    '\CompressionSettings.xml') then
  begin
    XMLDocument1.FileName := arcanumpath + '\Modules\' + modulefolder +
      '\CompressionSettings.xml';
...
begin
  ...
  XMLDocument1.FileName := arcanumpath + '\Modules\' + selectedmodule +
    '\CompressionSettings.xml';
...
begin
  modulesplitgroups := NewCompressionSettings;
  if fileexists(arcanumpath + '\Modules\' + selectedmodule +
    '\CompressionSettings.xml') then
  begin
    XMLDocument1.FileName := arcanumpath + '\Modules\' + selectedmodule +
      '\CompressionSettings.xml';
