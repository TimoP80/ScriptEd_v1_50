...
{$R *.res}
 ...
 // displayed in the menu of the host application
  ...
  procedure GenerateSpeech(text_male, text_female: PansiChar; voicename: PansiChar;
    outputpath: PansiChar; linenum: integer); stdcall;
  var
    theoutputfile: ansistring;
    ispeechflags: integer;
    str: Ansistring;
    voiceindex: integer;
  begin
    theoutputfile := 'v' + IntToStr(linenum) + '_m.wav';
    str:='MaleGenerateSpeech: ' + voicename + ' => ' + theoutputfile+' to path: '+outputpath;
    ...
    begin
      ...
      debugmessage(PAnsiChar('FemaleGenerateSpeech: ' + voicename + ' => ' + theoutputfile));
  ...
  begin
   try
    ...
    begin
      ...
      targetstr :=  AnsiString(zvoices.Item(u).getdescription(0));
      AddVoiceComboItem(pansichar(targetstr));
     end;
   DebugMessage(pansichar('Done.'));
   except on e:exception do
   DebugMessage(PAnsiChar('error: '+e.message));
   end;
  end;
