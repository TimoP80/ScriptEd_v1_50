...
uses
  ...
  SynEditSearch, VCL.Dialogs, VCL.Menus,
  ModuleLoader, dlgfileio, SynEditHighlighter,
  SynHighlighterGeneral, ComCtrls, StdCtrls,
  rxstrutils, MesFileIO, SynEdit, JclUnicode, pluginapi, JvExStdCtrls,
  JvShapedButton,
  ScriptEdConfig, JvExControls, JvDialButton, JvLED, JvSegmentedLEDDisplay,
  dwsCompiler, dwsDebugger,
  dwsExprs, dwsSymbols, dwsStack, dwsComp, dwsVCLGUIFunctions,
...
type
  TMainForm = class(TForm)
    ...
    DelphiWebScriptII1: TDelphiWebScript;
    dws2Unit1: TdwsUnit;
    dws2GUIFunctions1: TdwsGUIFunctions;
    Rumors1: TMenuItem;
    ...
    procedure ScriptEditorKeyDown(Sender: TObject; var Key: word;
      Shift: TShiftState);
    procedure ScriptEditorKeyUp(Sender: TObject; var Key: word;
      Shift: TShiftState);
    ...
    procedure JvMRUManager1Click(Sender: TObject;
    ...
    procedure dws2Unit1FunctionsSelectLineFromCurrentScriptEval
      (Info: TProgramInfo);
    procedure dws2Unit1FunctionsSelectBasicPrototypeEval(Info: TProgramInfo);
    procedure dws2Unit1FunctionsInitializeHelperScriptMenuEval
      (Info: TProgramInfo);
    ...
    procedure dws2Unit1VariablesModuleFolderReadVar(Info: TProgramInfo;
      var Value: Variant);
    procedure dws2Unit1VariablesModuleFolderWriteVar(Info: TProgramInfo;
      const [Ref] Value: Variant);
    procedure dws2Unit1VariablesArcanumPathReadVar(Info: TProgramInfo;
      var Value: Variant);
    procedure dws2Unit1VariablesArcanumPathWriteVar(Info: TProgramInfo;
      const [Ref] Value: Variant);
    procedure dws2Unit1VariablesEditorYPositionReadVar(Info: TProgramInfo;
      var Value: Variant);
    procedure dws2Unit1VariablesEditorYPositionWriteVar(Info: TProgramInfo;
      const [Ref] Value: Variant);
    procedure dws2Unit1FunctionsQuestionDlgEval(Info: TProgramInfo);
    procedure ConditionStatement1Click(Sender: TObject);
    procedure Generatedialogue1Click(Sender: TObject);
  private
    { Private declarations }
   public
     procedure RefreshScript;
     function TestOllamaCloudConnection(const Url, ApiKey: string): string;
     // Dialogue generation methods
     function GenerateNPCResponse(const NodeDescription, PlayerText: string): string;
     function GeneratePlayerOptions(const Context: string): TStringList;
     function GenerateJournalEntry(const Topic: string; Smart: Boolean): string;
     function GenerateSmartVersion(const Text: string): string;
     function GenerateDumbVersion(const Text: string): string;
   end;
...
uses
  DialogueGenerator, DialogueGeneratorForm, LoadOfficialScript, aboutbox, scriptproperties, DialogueEditor,
  ...
  SpeechGenInterface, SelectAction, SelectCondition, SelectCondAndStatement;
...
begin
  ...
  begin
    ...
    ConsoleDebug(format('Bass Version: %s', [BASSVERSIONTEXT]));
    BASS_GetDeviceInfo(1, bass_info);

    if BASS_Init(-1, 44100, 0, handle, nil) = True then
    begin
      ConsoleDebug(' ... Init successful!');
    end
    else
    begin
      ConsoleDebug(' ... Init failed! Error code: ' +
...
function SelectGlobalVariable: string;
begin
  form16.Caption := 'Select Global Variable';
  form16.Label1.Caption := 'List of Global Variables';
  fillindatafrommesfile(ScriptGlobalVars);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Result := IntToStr(ScriptGlobalVars.entries
      [form16.JvHTListBox1.itemindex].Index)
  else if form16.modalresult = mrCancel then
    Result := '';
end;

function SelectRumor: string;
begin
  form16.Caption := 'Select rumor / journal entry';
  form16.Label1.Caption := 'List of rumors / journal entries';
  fillindatafrommesfilewithdivisor(20, GameRD_npc_m2m);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Result := IntToStr(GameRD_npc_m2m.entries[form16.JvHTListBox1.itemindex].
      Index div 20)
  else if form16.modalresult = mrCancel then
    Result := '';
end;

function SelectQuest: string;
begin
  form16.Caption := 'Select quest';
  form16.Label1.Caption := 'List of Quests';
  fillindatafrommesfile(GameQuestLog);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Result := IntToStr(GameQuestLog.entries
      [form16.JvHTListBox1.itemindex].Index)
  else if form16.modalresult = mrCancel then
    Result := '';
end;

function SelectQuestState: string;
begin
  form16.Caption := 'Select quest state';
  form16.Label1.Caption := 'List of Quest states';
  fillindatafrommesfile(queststate);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Result := IntToStr(queststate.entries[form16.JvHTListBox1.itemindex].Index)
  else if form16.modalresult = mrCancel then
    Result := '';
end;

function SelectGlobalFlag: string;
begin
  form16.Caption := 'Select Global Flag';
  form16.Label1.Caption := 'List of Global Flags';
  fillindatafrommesfile(scriptglobalflags);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Result := IntToStr(scriptglobalflags.entries
      [form16.JvHTListBox1.itemindex].Index)
  else if form16.modalresult = mrCancel then
    Result := '';
end;
...
begin
  ...
  else if form18.modalresult = mrCancel then
    Result := '';
end;
...
//
// This is a universal function that parses an opcode line
// and uses prompts accordingly on per parameter basis
// Easier to do it like this than have the code duplicated for
// Script->Action, Script->Condition and Script->Condition + Statement
//

function ParseOpcodeLine(line: string; iscondition: boolean): string;
var
  temp: string;
  cmd: string;
  opcode: integer;
...
begin
  ...
  begin
    prmtype := GetOpcodeParamType(i, cmd);
    case prmtype of
      PARAM_TYPE_OBJ:
        begin
          replacestr := SelectFocus;
          temp := stringreplace(temp, '(obj)', replacestr, []);
        end;
      PARAM_TYPE_NUM:
        begin
          if iscondition = True then
          begin
            if (opcode = SC_GLOBAL_FLAG_NUM_IS_SET) then
              replacestr := SelectGlobalFlag
            else

              replacestr := SelectValue;
            temp := stringreplace(temp, '(num)', replacestr, []);

          end
          else
          begin
            if (opcode = SA_SET_GLOBAL_FLAG) or (opcode = SA_CLEAR_GLOBAL_FLAG)
            then
              replacestr := SelectGlobalFlag
            else if (opcode = SA_SET_RUMOR) then
            begin
              replacestr := SelectRumor;
            end
            else if (opcode = SA_SET_GLOBAL_QUEST_STATE) then
            begin
              if i = 1 then
                replacestr := SelectQuest
              else if i = 2 then
                replacestr := SelectQuestState;
            end
            else if (opcode = SA_SET_PC_QUEST_STATE) then
            begin
              if i = 2 then
                replacestr := SelectQuest
              else if i = 3 then
...
procedure ScriptLineAdd(str: PansiChar); stdcall;
begin
...
begin
  ConsoleDebug('Script Filename Update: ' + currentscript.filename);
  if script_changed = True then
    MainForm.Caption := 'ScriptEd 1.50.' + IntToStr(version_release) + ' - [' +
...
begin
  ...
  begin
    ...
    messagedlg(format('Error executing script %s:' + #13 + #10 + '' + #13 + #10
      + 'File does not exist!', [filename]), mtError, [mbOK], 0);
    exit;
  end;
...
begin
  try
    ...
    ScriptEditor.Lines.Text := stringreplace(ScriptEditor.Lines.Text, #13#10,
    ...
    on e: Exception do
    begin
      messagedlg('Error while updating script (' + e.message + ')', mtError,
        [mbOK], 0);
    end
  end;
...
function TMainForm.TestOllamaCloudConnection(const Url, ApiKey: string): string;
var
  CloudAPI: TOllamaCloudAPI;
  Models: TStringList;
begin
  Result := '';
  CloudAPI := nil;
  ...
  try
    try
      CloudAPI := TOllamaCloudAPI.Create(Url, ApiKey);
      if CloudAPI.IsServerRunning then
      begin
        Models := CloudAPI.ListModels;
        Result := 'Connection successful! ' + IntToStr(Models.Count) + ' models available.';
      end
      else
        Result := 'Connection failed: Server not reachable.';
    except
      on E: Exception do
...
function TMainForm.GenerateNPCResponse(const NodeDescription, PlayerText: string): string;
begin
  Result := TDialogueGenerator.GenerateNPCResponse(ollamarequest, ollamamodel, NodeDescription, PlayerText);
end;

function TMainForm.GeneratePlayerOptions(const Context: string): TStringList;
begin
  Result := TDialogueGenerator.GeneratePlayerOptions(ollamarequest, ollamamodel, Context);
end;

function TMainForm.GenerateJournalEntry(const Topic: string; Smart: Boolean): string;
begin
  Result := TDialogueGenerator.GenerateJournalEntry(ollamarequest, ollamamodel, Topic, Smart);
end;

function TMainForm.GenerateSmartVersion(const Text: string): string;
begin
  Result := TDialogueGenerator.GenerateSmartVersion(ollamarequest, ollamamodel, Text);
end;

function TMainForm.GenerateDumbVersion(const Text: string): string;
begin
  Result := TDialogueGenerator.GenerateDumbVersion(ollamarequest, ollamamodel, Text);
end;
...
begin
  ...
  begin
    case messagedlg
      (format('Save changes to script ''%s'' before creating a new one?',
...
begin
  ...
  SynAutoCorrect1.Items.loadfromfile(extractfiledir(ParamStr(0)) +
    '\autocorrect.lst');
  init_dat_logger(getcurrentdir);
  getfilelastwrite(ParamStr(0), date);
  builddate := datetimetostr(date);
  Newscript1Click(nil);
  // JvMRUManager1.LoadFromAppStorage(JvAppIniFileStorage1, '.\');
...
if ollamaenabled=true then
begin
  SplashMessage('Initializing Ollama API');
  if (OllamaProvider = 'OllamaCloud') then
  begin
    ollamarequest := TOllamaCloudAPI.Create(OllamaCloudUrl, OllamaCloudApiKey);
    if ollamarequest.IsServerRunning then
      SplashMessage('OllamaCloud connected.')
    else
      SplashMessage('OllamaCloud connection failed.');
  end
  else
  begin
    ollamarequest := TOllamaLocalAPI.Create('http://localhost:11434');
    if ollamarequest.IsServerRunning then
      SplashMessage('Ollama is running!')
    else
      SplashMessage('Ollama not running.');
  end;
end;
  ...
  SplashMessage('SoundLib Init');
  InitSound(MainForm.handle);

end;

procedure TMainForm.Quit1Click(Sender: TObject);
begin
  // JvMRUManager1.SaveToAppStorage(JvAppIniFileStorage1, '.\');
  // DelphiWebScriptII1.
    ...
    case OpenDialog1.FilterIndex of
      1:
        begin
          ...
          ChDir('..');
          if fileexists('dlg\' + changefileext
            (extractfilename(OpenDialog1.filename), '.dlg')) then
          begin
            DialogueEditor1.Enabled := True;
            LoadDialogue
              ('dlg\' + changefileext
              (extractfilename(OpenDialog1.filename), '.dlg'));
          end
          else
...
begin
  ...
  begin
    form1.Show;
    form1.filenamepanel.Caption := 'Filename: ' +
      extractfilename(currentscript.filename);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if currentscript.scriptlines[linenumadjust].thenPart.opcode <> SA_GOTO
            then
            begin
              ...
              begin
                ...
                begin
                  ...
                  while (currentscript.scriptlines[linenumadjust]
                    .thenPart.opcode <> SA_GOTO) and (linenumadjust >= 0) do
                  begin
                    linenumadjust := linenumadjust - 1;
                  end;
          ...
          begin
            ...
            if currentscript.scriptlines[linenumadjust].elsePart.opcode <> SA_GOTO
            then
            begin
              // consoledebug('THIS IS NOT THE FUCKING GOTO LINE going one backward!');
              linenumadjust := scriptlinerefs[s].linenum - 1;
              ...
              begin
                ...
                begin
                  // consoledebug('FINE! LETS FUCKING GO UP TO THE FIRST FUCKING LINE DAMMIT!');
                  while (currentscript.scriptlines[linenumadjust]
                    .elsePart.opcode <> SA_GOTO) and (linenumadjust >= 0) do
                  begin
                    linenumadjust := linenumadjust - 1;
                  end;
                  // consoledebug('all done.');
        ...
        else if oldlinecount < currentscript.LineCount then
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  while (currentscript.scriptlines[linenumadjust]
                    .thenPart.opcode <> SA_GOTO) and
                    (linenumadjust < currentscript.LineCount) do
                  begin
                    linenumadjust := linenumadjust + 1;
                  end;
                end;
          ...
          LINE_THEN:
            begin
              ...
              begin
                ...
                if scriptlinerefs[s].line_data_orig.opcode = currentscript.
                  scriptlines[linegoesto].thenPart.opcode then
                begin
                ...
                begin
                  ...
                  currentscript.scriptlines[scriptlinerefs[s].linenum]
                    .thenPart.varvalue[0] := lineadjust;
                end;
              ...
              begin
                ...
                begin
                  if (currentscript.scriptlines[z]
                    .thenPart.opcode = scriptlinerefs[s].line_data_orig.opcode)
                    and (currentscript.scriptlines[z].thenPart.VarTypes[0]
            ...
            begin
              ...
              begin
                ...
                begin
                  scriptlinerefs[s].linenum := linenumadjust;
                  currentscript.scriptlines[scriptlinerefs[s].linenum]
                    .elsePart.varvalue[0] := lineadjust;
                end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if getfileindex(actionopcodesdat,
        '\dlg\' + extractfilename(changefileext(selectedfile, '.dlg'))) <> -1
      then
      begin
        // consoledebug('found patched');
        openfilefromdat(actiondathandle, actionopcodesdat,
          '\dlg\' + extractfilename(changefileext(selectedfile, '.dlg')),
          extractfilename(changefileext(selectedfile, '.dlg')));
      end
      else
      begin
        // consoledebug('not patched, defaulting to arcanum3.dat');
        openfilefromdat(dathandle, opcodesdat,
          '\dlg\' + extractfilename(changefileext(selectedfile, '.dlg')),
          arcanumpath + 'data\dlg\' + extractfilename
          (changefileext(selectedfile, '.dlg')));
      end;
...
{$IFDEF BETARELEASE}
  form5.Label4.Caption :=
    'Note: This is a beta release. This means that not everything has been thoroughly tested. If you encounter a bug, please report it by e-email at tpitkane@gmail.com';
{$ENDIF}
  form5.Show;

end;

procedure GetModuleDir(var Data: PansiChar); stdcall;
begin
  Getmem(Data, length(arcanumpath + '\Modules\' + modulefolder) + 1);
  StrPCopy(Data, arcanumpath + '\Modules\' + modulefolder);
  // Data := pansichar(arcanumpath + '\Modules\' + modulefolder);
  ConsoleDebug('returned module dir as ' + ansistring(Data) +
    ' modulefolder is ' + modulefolder);
end;
...
procedure AddVoiceComboItem(str: PansiChar); stdcall;
...
begin
  // consoledebug('Setting script pointer');
  scrptr := PScriptFile(currentscript);
end;

procedure DebugMessage(Data: PansiChar); stdcall;
begin
  ConsoleDebug(Data);
end;
...
exports GetDialogue, AddVoiceComboItem, ClearVoiceComboItems, EditorFindLine,
  GetModuleDir, DebugMessage, GetScript, ScriptLineAdd;

procedure TMainForm.Properties1Click(Sender: TObject);
begin

  form4.nonmagictrap.Checked := (currentscript.scriptflags and
    FLAGS_NONMAGICAL_TRAP) <> 0;
  form4.magictrap.Checked := (currentscript.scriptflags and
    FLAGS_MAGICAL_TRAP) <> 0;
  form4.radius2.Checked := (currentscript.scriptflags and
    FLAGS_RADIUS_TWO) <> 0;
  form4.radius3.Checked := (currentscript.scriptflags and
    FLAGS_RADIUS_THREE) <> 0;
  form4.radius5.Checked := (currentscript.scriptflags and
    FLAGS_RADIUS_FIVE) <> 0;
  form4.teleporttrig.Checked := (currentscript.scriptflags and
    FLAGS_TELEPORT_TRIGGER) <> 0;
  form4.deathspeech.Checked := (currentscript.scriptflags and
    FLAGS_DEATH_SPEECH) <> 0;
  form4.autoremove.Checked := (currentscript.scriptflags and
    FLAGS_AUTO_REMOVING) <> 0;
  ...
  begin
    ...
    if form4.nonmagictrap.Checked then
      currentscript.scriptflags := currentscript.scriptflags +
        FLAGS_NONMAGICAL_TRAP;
    ...
    if form4.teleporttrig.Checked then
      currentscript.scriptflags := currentscript.scriptflags +
        FLAGS_TELEPORT_TRIGGER;

    if form4.surrspeech.Checked then
      currentscript.scriptflags := currentscript.scriptflags +
        FLAGS_SURRENDER_SPEECH;
  ...
  begin
    case SaveDialog1.FilterIndex of
      1:
        begin
          Compile1Click(nil);
          currentscript.script_id := script_id;
          currentscript.filename :=
            format('%0.5d%s', [currentscript.script_id,
            extractfilename(SaveDialog1.filename)]);
          ConsoleDebug('Current Script filename = ' + currentscript.filename);
          SaveScript(format('%s\%0.5d%s', [extractfiledir(SaveDialog1.filename),
            currentscript^.script_id, extractfilename(SaveDialog1.filename)]),
            currentscript^);
          RefreshScript;
...
begin
  ...
  if DirectoryExists(arcanumpath + '\Modules\' + modulefolder + '\Sound\Speech\'
    + copy(extractfilename(currentscript.filename), 1, 5)) then
  begin
    ConsoleDebug('VoiceOverFolder "' + voicedir +
      '" exists - disable create button');
...
form3.OllamaGenerate.Enabled := ollamaenabled;
...
begin
  ...
  begin
    ...
    begin
      ...
      Module1.Caption := 'Module: ' + modulefolder;
      Module1.Visible := True;
      OpenDialog1.InitialDir := arcanumpath + '\Modules\' + modulefolder
        + '\scr\';
      SaveDialog1.InitialDir := arcanumpath + '\Modules\' + modulefolder
        + '\scr\';
    end;
...
var
  line: string;
begin
  line := Info.Vars['linedata'].ValueAsString;
...
begin
  ConsoleDebug(Info.Vars['data'].ValueAsString);
end;
...
begin
  ...
  begin
    ...
    if (pos('call script', linetext) <> 0) and (pos('attached to', linetext) = 0)
    then
    begin
      wordcnt := wordcount(linetext, [' ', ':', ',']);
      for i := 1 to wordcnt do
    ...
    if (ScriptEditor.WordAtMouse = 'Flag') or
      (ScriptEditor.WordAtMouse = 'Variable') then
    begin
      wordcnt := wordcount(linetext, [' ', ':', ',']);
      for i := 1 to wordcnt do
      begin
    ...
    begin
      ...
      if modulefolder = 'Arcanum' then
        AdvBuildFileList(arcanumpath + '\data' + format('\scr\%0.5d*.scr',
          [wordparam]), faAnyFile, scriptslst, amAny)
      else
    ...
    begin
      ...
      begin
        ...
        if CurDLG.nodes[nodeind].npctextfemale <> CurDLG.nodes[nodeind].npctextmale
        then
        begin
          textdata := 'Dialogue line ' + IntToStr(wordparam) + ':' +
            #13#10'Male PC - ' + #13#10#13#10 + dlgdata + #13#10#13#10 +
            'Female PC - ' + #13#10 + #13#10 + CurDLG.nodes[nodeind]
            .npctextfemale;
        end
        else

          textdata := 'Dialogue line ' + IntToStr(wordparam) + ': ' + #13#10
            + dlgdata;
      end;
    ...
    begin
      ...
      textdata := textdata + #13#10 + #13#10 + GetMesStringByID(wordparam,
        AttachmentPointDesc);
    end;
...
begin
  ...
  CompilerLog.Lines.SaveToFile('ScriptEd.log');
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DialogueEditor1.Enabled := False;
      CurDLG.nodecount := 0;
    end;

    RefreshScript;
    RequestPluginRefresh;
    script_id := StrToInt(copy(selectedmodulescript, 1, 5));
    ConsoleDebug('Script ID = ' + IntToStr(script_id));
    undobuffer := ScriptEditor.Lines.Text;
    SetCurrentDir(extractfiledir(ParamStr(0)));
    JvMRUManager1.Add(arcanumpath + '\Modules\' + modulefolder + '\scr\' +
      selectedmodulescript, 0);
  end;
...
begin
  ...
  begin
    messagedlg
      ('You must save your script first before creating a dialogue file.',
      mtInformation, [mbOK], 0);
    exit;
  end;
  DialogueFileName := arcanumpath + '\Modules\' + modulefolder + '\dlg\' +
    changefileext(currentscript.filename, '.dlg');

  if fileexists(DialogueFileName) = True then
  begin
    case messagedlg(format('This action will overwrite the existing ' + #13 +
      #10 + 'dialogue file ''%s''' + #13 + #10 + '' + #13 + #10 +
      'Do you wish to continue?', [DialogueFileName]), mtConfirmation,
      [mbYes, mbNo], 0) of
      mrNo:
        exit;
    end;
  ...
  begin
    ...
    begin
      tempsave.Add('//');
      tempsave.Add('// <<SCRIPT>>: ' +
        stringreplace(decode_script_line(currentscript.scriptlines[t]^), #13#10,
        #13#10 + '// ', [rfReplaceAll]));
    end;
    ...
    begin
      ...
      begin
        ...
        tempsave.Add('// Line ' + IntToStr(t) + ' - float message ' +
          IntToStr(dlgnum));
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        tempsave.Add('// Line ' + IntToStr(t) + ' - dlg line ' +
          IntToStr(dlgnum));
      end;
    end;
  ...
  BubbleSort(dlglines);
  if dlglinecnt = 0 then
  begin
    ConsoleDebug
      ('Script had no entrypoints. Adding default line 1 to the dialogue.');
    Add_DLGLine(1);
  end;
  ...
  tempsave.SaveToFile(arcanumpath + '\Modules\' + modulefolder + '\dlg\' +
    changefileext(currentscript.filename, '.dlg'));
  CurDLG.dlgfilename := changefileext(currentscript.filename, '.dlg');
  messagedlg(format('Dialogue file created as:' + #13 + #10 + '' + #13 + #10 +
    '%s', [arcanumpath + '\Modules\' + modulefolder + '\dlg\' +
    changefileext(currentscript.filename, '.dlg')]), mtInformation, [mbOK], 0);
  tempsave.Free;
  DialogueEditor1.Enabled := True;
...
begin
  LoadPASScript(extractfiledir(ParamStr(0)) + '\' + templatescripts
    [TMenuItem(Sender).tag].filename);
end;
...
begin
  ...
  form15.enableollama.Checked := ollamaenabled;
  form15.confirmplayeroptiondelete.Checked := confirmplayeroptiondelete;
  form15.AutoIncrementVONumber.Checked := AutoIncrementVONumber;
   form15.AutoExpand.Checked:=AutoExpand;
   // Set provider selection
   form15.ProviderComboBox.ItemIndex := form15.ProviderComboBox.Items.IndexOf(OllamaProvider);
   // Trigger visibility update
   form15.ProviderComboBoxChange(nil);
   // Set cloud credentials
   form15.EditCloudApiKey.Text := OllamaCloudApiKey;
   form15.EditCloudUrl.Text := OllamaCloudUrl;
   if OllamaEnabled=True then
   begin
     ConsoleDebug('Ollama checking models..');
     form15.ollamamodellist.Items := ollamarequest.ListModels;
     consoledebug(inttostr(form15.ollamamodellist.Items.count)+' models available.');
     form15.ollamamodellist.ItemIndex :=form15.ollamamodellist.items.IndexOf(ollamamodel);
   end;
  form15.dbmaker.Checked := UseDBMaker;
  refreshgenderstrings;
  RefreshHelperScripts;
  form15.showmodal;
   if form15.modalresult = mrOk then
   begin
     ollamamodel := form15.ollamamodellist.Text;
     autoexpand :=form15.AutoExpand.Checked;
     ...
     ollamaenabled := form15.enableollama.Checked;
     ...
     AutoIncrementVONumber := form15.AutoIncrementVONumber.Checked;
     UseDBMaker := form15.dbmaker.Checked;
     // Read provider and cloud settings
     OllamaProvider := form15.ProviderComboBox.Text;
     if OllamaProvider = '' then OllamaProvider := 'Local';
     OllamaCloudApiKey := form15.EditCloudApiKey.Text;
     OllamaCloudUrl := form15.EditCloudUrl.Text;
     WriteHelperScriptLoader;
     LoadPASScript(extractfiledir(ParamStr(0)) +
       '\helperscripts\HelperScripts.dws');
     SaveConfig;
   end;
...
begin
  ...
  if fileexists(extractfiledir(ParamStr(0)) + '\' + Info.Vars['Filename']
    .ValueAsString) = False then
  begin
    ConsoleDebug(format('Warning: Supplied helper script "%s" does not exist.',
      [Info.Vars['Filename'].ValueAsString]));
  end;
  setlength(templatescripts, tempscnt + 1);
  new(templatescripts[tempscnt]);
  templatescripts[tempscnt].filename := Info.Vars['Filename'].Value;
  templatescripts[tempscnt].description := Info.Vars['Description'].Value;
  templatescripts[tempscnt].category := Info.Vars['Category'].Value;
  ...
  category.Caption := Info.Vars['Category'].Value;
  if HelperScripts1.find(Info.Vars['Category'].Value) = nil then
    HelperScripts1.Add(category)
  else
    category := HelperScripts1.find(Info.Vars['Category'].Value);
  category.Add(s);
  Inc(tempscnt);
  if tempscnt > 0 then
    MainForm.HelperScripts1.Visible := True
  else
    MainForm.HelperScripts1.Visible := False;
...
begin
  ...
  begin
    temp := ParseOpcodeLine(selectedcommand, False);
    ScriptEditor.Lines.Add(temp);
...
begin
  ...
  begin
    temp := ParseOpcodeLine(selectedcondition, True);
    ScriptEditor.Lines.Add('IF ' + temp);
  end;

end;

procedure TMainForm.ConditionStatement1Click(Sender: TObject);
var
  ifline, thenline, elseline: string;
var
  ifresult, thenresult, elseresult: string;
begin
  UpdateLists;
  form27.showmodal;
  if form27.modalresult = mrOk then
  begin
    ifline := form27.condlist.Text;
    thenline := form27.thenlist.Text;
    elseline := form27.elselist.Text;

    if elseline = 'do nothing' then
    begin
      ifresult := ParseOpcodeLine(ifline, True);
      thenresult := ParseOpcodeLine(thenline, False);
      ScriptEditor.Lines.Add('IF ' + ifresult);
      ScriptEditor.Lines.Add('THEN ' + thenresult);
    end
    else
    begin
      ifresult := ParseOpcodeLine(ifline, True);
      thenresult := ParseOpcodeLine(thenline, False);
      elseresult := ParseOpcodeLine(elseline, False);

      ScriptEditor.Lines.Add('IF ' + ifresult);
      ScriptEditor.Lines.Add('THEN ' + thenresult);
      ScriptEditor.Lines.Add('ELSE ' + elseresult);
...
procedure TMainForm.Generatedialogue1Click(Sender: TObject);
begin
  if not Assigned(ollamarequest) then
  begin
    ShowMessage('Ollama is not enabled or not configured. Please enable and configure Ollama in Preferences first.');
    Exit;
  end;
  TFormDialogueGen.Execute(ollamarequest, ollamamodel);
end;

procedure DoScriptCommand(opcode, param1, param2, param3, param4, param5,
  param6, param7, param8: integer; Lines: TStrings; position: integer = -1);
var
  line: string;
begin
  ...
  case opcode of
    ...
    SA_CALL_SCRIPT:
      line := 'call script ' + IntToStr(param1) + ' at line ' + IntToStr(param2)
        + ' with triggerer ' + GetMesStringByID(param3, focusdata) +
    ...
    SA_SET_GLOBAL_QUEST_STATE:
      line := 'set quest ' + IntToStr(param1) + ' to global state ' +
        IntToStr(param2);
...
begin
  opcode := Info.Vars['opcode'].ValueAsInteger;
  param1 := Info.Vars['param1'].ValueAsInteger;
  param2 := Info.Vars['param2'].ValueAsInteger;
  param3 := Info.Vars['param3'].ValueAsInteger;
  param4 := Info.Vars['param4'].ValueAsInteger;
  param5 := Info.Vars['param5'].ValueAsInteger;
  param6 := Info.Vars['param6'].ValueAsInteger;
  param7 := Info.Vars['param7'].ValueAsInteger;
  param8 := Info.Vars['param8'].ValueAsInteger;
  position := Info.Vars['pos'].ValueAsInteger;
  DoScriptCommand(opcode, param1, param2, param3, param4, param5, param6,
    param7, param8, ScriptEditor.Lines, position);
...
var
  newscriptid: integer;
begin
  newscriptid := StrToInt(inputbox('Enter Script ID', 'Prompt',
    IntToStr(script_id)));
  script_id := newscriptid;
  ConsoleDebug('New Script ID = ' + IntToStr(script_id));

  // messagedlg('Not implemented yet.' + #13 + #10 + '' + #13 + #10 +
  // 'Script ID should currently be written' + #13 + #10 +
  // 'manually when you save the script' + #13 + #10 + 'file.' + #13 + #10 + '' +
  // #13 + #10 + 'This will be sorted out later to correspond ' + #13 + #10 +
  // 'the behaviour of the old ScriptEd.', mtInformation, [mbOK], 0);
end;
...
begin
  ...
  ChDir('..');
  if fileexists('dlg\' + changefileext(extractfilename(RecentName), '.dlg'))
  then
  begin
    DialogueEditor1.Enabled := True;
...
begin
  form16.Caption := 'Select Global Flag';
  form16.Label1.Caption := 'List of global flags in module "' +
    modulefolder + '"';
  fillindatafrommesfile(scriptglobalflags);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := scriptglobalflags.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := ScriptGlobalVars.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := GameQuestLog.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := GameRD_npc_m2m.entries
      [form16.JvHTListBox1.itemindex].Index div 20
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
begin
  ...
  begin
    ...
    constind := TDWSConstant(dws2Unit1.Constants.Items[X]).Value;
...
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := ScriptPCFlags.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := ScriptPCVars.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
procedure TMainForm.dws2Unit1FunctionsSelectInternalNameEval
  (Info: TProgramInfo);
begin
  form16.Caption := 'Select internal name';
  form16.Label1.Caption := 'List of internal names in module "' +
    modulefolder + '"';
  fillindatafrommesfile(GameOName);
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := GameOName.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
begin
  ...
  begin
    ...
    begin
      ...
      script := -2;
    Info.Vars['FileNameStorage'].Value := selectedmodulescript;
    Info.Vars['result'].Value := script;
...
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := focusdata.entries
      [form16.JvHTListBox1.itemindex].messagestr
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := '';
end;
...
begin
  ...
  begin
    if form18.JvHTListBox1.itemindex = 3 then
      Info.Vars['result'].Value := IntToStr(form18.ValueData.AsInteger)
    else

      Info.Vars['result'].Value := ValueData.entries
        [form18.JvHTListBox1.itemindex].messagestr + ' ' +
        IntToStr(form18.ValueData.AsInteger);
  end
  else if form18.modalresult = mrCancel then
    Info.Vars['result'].Value := '';
...
begin
  ShowScriptLines(Info.Vars['srcfile'].Value);
  form19.showmodal;

  if form19.modalresult = mrOk then
  begin
    Info.Vars['result'].Value := selectedline;
  end
  else
    Info.Vars['result'].Value := -1;
...
procedure TMainForm.dws2Unit1FunctionsSelectBasicPrototypeEval
  (Info: TProgramInfo);
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := Prototypes.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
...
procedure TMainForm.dws2Unit1FunctionsInitializeHelperScriptMenuEval
  (Info: TProgramInfo);
begin
  tempscnt := 0;
...
procedure TMainForm.dws2Unit1FunctionsGetEditorCursorPositionEval
  (Info: TProgramInfo);
begin
  Info.Vars['result'].Value := ScriptEditor.CaretY;
end;

procedure TMainForm.dws2Unit1FunctionsInsertScriptLineEval(Info: TProgramInfo);
begin
  ScriptEditor.Lines.Insert(Info.Vars['pos'].Value, Info.Vars['line'].Value);
end;

procedure TMainForm.dws2Unit1FunctionsQuestionDlgEval(Info: TProgramInfo);
begin
  if messagedlg(Info.Vars['Question'].Value, mtConfirmation, [mbYes, mbNo], 0) = mrYes
  then
    Info.Vars['result'].Value := True
  else
    Info.Vars['result'].Value := False;

end;

procedure TMainForm.dws2Unit1FunctionsShowScriptInfoEval(Info: TProgramInfo);
begin
  form21.scriptname.Caption := Info.Vars['name'].Value;
  form21.description.Caption := Info.Vars['description'].Value;
  form21.author.Caption := Info.Vars['author'].Value;
  form21.Infotext.Text := Info.Vars['infotext'].Value;
  form21.showmodal;

end;

procedure TMainForm.dws2Unit1VariablesArcanumPathReadVar(Info: TProgramInfo;
  var Value: Variant);
begin
  Value := arcanumpath
end;

procedure TMainForm.dws2Unit1VariablesArcanumPathWriteVar(Info: TProgramInfo;
  const [Ref] Value: Variant);
begin
  arcanumpath := Value;
end;

procedure TMainForm.dws2Unit1VariablesEditorYPositionReadVar(Info: TProgramInfo;
  var Value: Variant);
begin
  Value := ScriptEditor.CaretY;
end;

procedure TMainForm.dws2Unit1VariablesEditorYPositionWriteVar
  (Info: TProgramInfo; const [Ref] Value: Variant);
begin
  ScriptEditor.CaretY := Value;
end;

procedure TMainForm.dws2Unit1VariablesModuleFolderReadVar(Info: TProgramInfo;
  var Value: Variant);
begin
  Value := modulefolder;
end;

procedure TMainForm.dws2Unit1VariablesModuleFolderWriteVar(Info: TProgramInfo;
  const [Ref] Value: Variant);
begin
  modulefolder := Value;
end;
...
begin
  ...
  begin
    ...
    MesFileGeneric := ParseMES(arcanumpath + '\Modules\' + modulefolder + '\'
      + thefile);
    ShowMSGData(MesFileGeneric);
    form7.showmodal;
    if form7.modalresult = mrOk then
...
procedure TMainForm.dws2Unit1FunctionsChooseFocus_IntegerEval
  (Info: TProgramInfo);
begin
  form16.Caption := Info.Vars['Caption'].Value;
...
procedure TMainForm.dws2Unit1FunctionsSelectScriptCallTypeEval
  (Info: TProgramInfo);
var
  tempdata: integer;
begin
  ...
  begin
    ...
    case tempdata of
      0:
        Info.Vars['result'].Value := SA_CALL_SCRIPT;
      1:
        Info.Vars['result'].Value := SA_CALL_SCRIPT_IN_SECONDS;
      2:
        Info.Vars['result'].Value := SA_CALL_SCRIPT_AT_SECOND;
      3:
        Info.Vars['result'].Value := SA_CALL_SCRIPT_ATTACHED_TO;
    end;
...
procedure TMainForm.dws2Unit1FunctionsSelectAttachmentPointEval
  (Info: TProgramInfo);
begin
  ...
  form16.showmodal;
  if form16.modalresult = mrOk then
    Info.Vars['result'].Value := AttachmentPoints.entries
      [form16.JvHTListBox1.itemindex].Index
  else if form16.modalresult = mrCancel then
    Info.Vars['result'].Value := -1;
end;
