...
begin
  ...
  try
    ...
    begin
      ...
      ShowMessage('Error adding file: ' + filename + #13#10 + 'Current directory: ' + getcurrentdir + #13#10 +
...
begin
  ...
  try
    ...
    on e: Exception do
    begin
      ShowMessage('DATLib Exception:' + #13#10 + e.message + #13#10 + 'While trying to extract a file with full path.');
    end;
  end;
...
begin
  try
    ...
    begin
      case MessageDlg('Warning! DAT File count is weird.' + #13 + #10 + 'Do you wish to manually enter the filecount?',
    ...
    begin
      if isconsole = False then
        MessageDlg(format('Error while attempting to open DAT file %s', [filename]), mtError, [mbOK], 0);
