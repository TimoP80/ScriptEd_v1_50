...
interface
 ...
 // DAT creation functions work correctly now, finally
 // --------------------------------------------------
...
resourcestring
  Sig = 'ArcDATLib 1.01 - (c) 2008 by DjUnique';
...
type
  datfileheader = record
    ...
    treepos:                           Integer;
    totalsize:                         Integer;
    dat_filename:                      AnsiString;
    files:                             array of ^datfilerec;
    unknown1, junk1, junk2, signature: Integer;
...
var
  ...
  dlogfile, catchmsg:                           AnsiString;
  dlog:                                         textfile;
  logging_enabled:                              Boolean;
  chars:                                        array [0..255] of AnsiChar;
...
procedure openfilefromdat(var f: file; var datrec: datfileheader; filename: ansiString; alt_filename: ansiString);
...
function folderexists(datrec: datfileheader; folder: String): Boolean;
procedure decompress_as(infile, newfile: String);
function compress_(filename: String; compressionratio: TCompressionlevel): Longint;
// for data save functions

implementation

//uses progressdlg, mainfrm;
uses arcanumscrlibcmdline, Math;
...
procedure WindowLogMessage(str: String);
begin
  ...
  try
    ...
    try
      ...
      try
        ...
        ZStream := TDecompressionStream.Create(_InFile);
        try
          while True do
          begin
...
begin
  ...
  begin
    ...
    blockread(f, cachefiles.files[i].unknown1, 4);
...
begin
  ...
  WindowLogMessage('Writing dat header for ' + datrec.dat_filename + ' filecount = ' + IntToStr(datrec.filecount));
  ...
  begin
    ...
    datrec.files[i].Filenamesize := datrec.files[i].Filenamesize + 1;
    blockwrite(xf, datrec.files[i].Filenamesize, 4);
    for j := 1 to datrec.files[i].filenamesize - 1 do
    begin
    ...
    blockwrite(xf, datrec.files[i].unknown1, 4);
  ...
  datrec.signature := 1145132081;
  //  datrec.junk1 := 0;
  //  datrec.junk2 := 0;

  //crc32(
  blockwrite(xf, datrec.unknown1, 4);
  blockwrite(xf, datrec.junk1, 4);
  blockwrite(xf, datrec.junk2, 4);
  blockwrite(xf, datrec.junk2, 4);
  blockwrite(xf, datrec.signature, 4);
  ...
  dat_log_msg('wrote dirtreesize2 : ' + IntToStr(dirtreesize2));
  dat_log_msg('wrote datrec.dirtreesize : ' + IntToStr(datrec.dirtreesize));
...
begin
  ...
  compressor := TCompressionStream.Create(compressionratio, dest_stream);
...
procedure add_folder_to_dat(var xf: file; var datrec: datfileheader; filename: String);
begin
  dat_log_msg('Adding folder ' + filename);
  if filename = '' then
  begin
    dat_log_msg('WTF!!! NULL FOLDER!');
    exit;
  end;
  WindowLogMessage('Creating folder ' + filename);
  setlength(datrec.files, datrec.filecount + 1);
  new(datrec.files[datrec.filecount]);
  datrec.files[datrec.filecount].filename := '';
  datrec.files[datrec.filecount].filename := filename;
  ...
  datrec.files[datrec.filecount].unknown1 := 0;
  datrec.files[datrec.filecount].itemtype := FILETYPE_DIR;
  datrec.files[datrec.filecount].offset := 0;
  Inc(datrec.dirtreesize, 4 + datrec.files[datrec.filecount].Filenamesize + 1 + 4 + 4 + 4 + 4 + 4);
...
var
  f: file;
  fnametemp: String;
  base, dirnode, dirnodemain, tempstring: String;
  temp_pchar: PChar;
...
begin
  ...
  begin
    dirnodecount := wordcount(extractfiledir(filename), ['\']);
    base := extractword(1, extractfiledir(filename), ['\']);
    if folderexists(datrec, base) = False then
      add_folder_to_dat(xf, datrec, base);
    if dirnodecount > 1 then
    begin
      for n := 2 to dirnodecount do
      begin
        base := base + '\' + extractword(n, extractfiledir(filename), ['\']);
        if folderexists(datrec, base) = False then
          add_folder_to_dat(xf, datrec, base);
      end;
  ...
  try
    ...
    writeln('Error adding file: ' + filename + #13#10 + 'Current directory: ' + getcurrentdir + #13#10 +
        'Exception: ' + #13 + #10 + e.message);
      exit;
    end;
  end;
  ...
  Close(f);
  Compress_(filename, clMax);
  Assign(f, extractfilename(filename) + '.zlib');
  reset(f, 1);
  datrec.files[datrec.filecount].packedsize := filesize(f);

  WindowLogMessage('Added ' + filename + ' ' + IntToStr(datrec.files[datrec.filecount].realsize) + ' -> ' +
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        str := str + 'File_Not_Found: ' + startupdir + '\' + extractfilename(datrec.files[j].filename) + #13#10;
      end
      else
      begin
...
var
  savepos: Integer;
begin
  //dat_log_msg('File attempt to file '+filename);
  ind := getfileindex(datrec, filename);
  if ind = -1 then
  begin
  // dat_log_msg('Perseen vittu not found');
    exit;
  end;
  dat_log_msg('LoadFile ' + filename + ': index=' + IntToStr(ind));
  ...
  begin
    dat_log_msg('File is compressed');
  ...
  begin
    dat_log_msg('File is not compressed');
    if alt_filename <> '' then
      Assign(outf, alt_filename)
    else
  ...
  begin
    ...
    begin
      dat_log_msg('FILENAME_OVERRIDE: ' + alt_filename);
      if extractfiledir(alt_filename) <> '' then
      begin
        decompress_as(extractfilename(filename) + '.zlib', alt_filename);
        if fileexists(alt_filename) = False then
        begin
          //     dat_log_msg('FATAL HOMO ERROR! FILE IS NOT THERE!');
        end;
...
begin
  ...
  try
    ...
    begin
      ;
      writeln('DAT File entry ' + IntToStr(ind) + ' is null.' + #13#10 + 'Contact the author. This should NEVER happen.');
      // this won''t happen
    end;
    ...
    on e: Exception do
    begin
      writeln('DATLib Exception:' + #13#10 + e.message + #13#10 + 'While trying to extract a file with full path.');
    end;
  end;
...
begin
  try
    ...
    blockread(f, datrec.junk1, 4);

    blockread(f, datrec.junk2, 4);

    blockread(f, datrec.signature, 4);
    seek(f, filesize(f) - treesubs);
    blockread(f, whatsthere, 4);
    ...
    setlength(datrec.files, datrec.filecount);
    ...
    begin
      ...
      blockread(f, chars, datrec.files[y].Filenamesize);
      datrec.files[y].filename := chars;
      blockread(f, datrec.files[y].unknown1, 4);
      blockread(f, datrec.files[y].itemtype, 4);
      blockread(f, datrec.files[y].realsize, 4);
      blockread(f, datrec.files[y].packedsize, 4);
      blockread(f, datrec.files[y].offset, 4);
    ...
    on e: Exception do
    begin
       writeln(format('Error while attempting to open DAT file %s', [filename]));
