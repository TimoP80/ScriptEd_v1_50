...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
   arcanumscrlib, ScriptEdConfig, Dialogs, StdCtrls, Mask, JvExMask,
   JvToolEdit,
   ComCtrls, ExtCtrls,
   dwsCompiler, DwsInfo, dwsDebugger,
   dwsExprs, dwsSymbols, dwsStack, dwsComp,
   JvSpin;
...
type
  TForm15 = class(TForm)
    ...
    dbmaker: TCheckBox;
    AutoIncrementVONumber: TCheckBox;
    TabSheet3: TTabSheet;
    ollamamodellist: TComboBox;
    Label10: TLabel;
    enableollama: TCheckBox;
    AutoExpand: TCheckBox;
    Label11: TLabel;
    ProviderComboBox: TComboBox;
    LabelCloudApiKey: TLabel;
    EditCloudApiKey: TEdit;
    LabelCloudUrl: TLabel;
    EditCloudUrl: TEdit;
    ButtonTestCloud: TButton;
    procedure ListView1Click(Sender: TObject);
    procedure Button3Click(Sender: TObject);
    procedure Button4Click(Sender: TObject);
    procedure maledataKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure femaledataKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure FormCreate(Sender: TObject);
    procedure HelperScriptListClick(Sender: TObject);
    procedure scriptfilenameAfterDialog(Sender: TObject; var AName: String;
      var AAction: Boolean);
    procedure Button5Click(Sender: TObject);
    procedure descriptionKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure categorycomboClick(Sender: TObject);
    procedure Button6Click(Sender: TObject);
    procedure enableollamaClick(Sender: TObject);
    procedure ProviderComboBoxChange(Sender: TObject);
    procedure ButtonTestCloudClick(Sender: TObject);
  private
...
begin
  ...
  begin
    strs.add(format('AddHelperScript(''%s'',''%s'',''%s'');',
      [templatescripts[t].filename, templatescripts[t].description,
  ...
  strs.add('');
  strs.SaveToFile(extractfiledir(ParamStr(0)) +
...
procedure TForm15.maledataKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if ListView1.selected <> nil then
  begin
...
procedure TForm15.femaledataKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if ListView1.selected <> nil then
  begin
...
begin
  ...
  ProviderComboBox.Items.Add('Local Ollama');
  ProviderComboBox.Items.Add('OllamaCloud');
end;
...
begin
  ...
  begin
    scriptfilename.filename := templatescripts[HelperScriptList.selected.
      index].filename;
    description.Text := templatescripts[HelperScriptList.selected.index]
      .description;
    categorycombo.ItemIndex := categorycombo.items.indexof
      (templatescripts[HelperScriptList.selected.index].category);
  end
  else
  begin
...
procedure TForm15.scriptfilenameAfterDialog(Sender: TObject; var AName: String;
  var AAction: Boolean);
var
  x, ind: Integer;
  resultinf: IInfo;
...
begin
  ...
  begin
    ...
    begin
      Consoledebug
        ('This script is importable, calling the ImportScriptProcedure');
      ...
      begin
        ...
        begin
          Consoledebug(script_import.Msgs[x].Text);
        end;

        exit;
      end;
      exec := script_import.CreateNewExecution;
      exec.BeginProgram;
      inf := exec.info.Func['ImportScript'];
      resultinf := inf.Call;
...
begin
  if (scriptfilename.filename <> '') and (description.Text <> '') and
    (categorycombo.ItemIndex <> -1) then
  begin
...
procedure TForm15.descriptionKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  ind: Integer;
begin
...
procedure TForm15.enableollamaClick(Sender: TObject);
begin
  (* old code *)
end;

procedure TForm15.ProviderComboBoxChange(Sender: TObject);
begin
  // Show/hide cloud-specific fields based on provider selection
  if ProviderComboBox.Text = 'OllamaCloud' then
  begin
    LabelCloudApiKey.Visible := True;
    EditCloudApiKey.Visible := True;
    LabelCloudUrl.Visible := True;
    EditCloudUrl.Visible := True;
    ButtonTestCloud.Visible := True;
  end
  else
  begin
    LabelCloudApiKey.Visible := False;
    EditCloudApiKey.Visible := False;
    LabelCloudUrl.Visible := False;
    EditCloudUrl.Visible := False;
    ButtonTestCloud.Visible := False;
  end;
end;

procedure TForm15.ButtonTestCloudClick(Sender: TObject);
var
  Msg: string;
begin
  if Trim(EditCloudApiKey.Text) = '' then
  begin
    ShowMessage('Please enter an API key.');
    Exit;
  end;
  Msg := MainForm.TestOllamaCloudConnection(EditCloudUrl.Text, EditCloudApiKey.Text);
  ShowMessage(Msg);
end;
