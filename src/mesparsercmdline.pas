unit mesparsercmdline;

{ MESParser
  Version: 0.0 Release 0 Build 0
  Fallout 2 MSG Parser
  Date of Generation: 9.7.2008 23:03
  Author: T. Pitkänen
  (c) 2008 T. Pitkänen
...
type
  ...
  EMESParser = class(Exception);
  TMESParser = class;

  TMESParserScanner = class(TCocoRScanner)
  private
    FOwner:           TMESParser;
  ...
  public
    ...
    property Owner: TMESParser Read fOwner Write fOwner;
    property ScannerError;
    property SrcStream;
  end;  { TMESParserScanner }

  TMESParser = class(TCocoRGrammar)
  private
  ...
  public
    ...
    procedure Execute;
    function GetScanner: TMESParserScanner;
    procedure Parse;
...
function PadR(S: AnsiString; Ch: AnsiChar; L: Integer): AnsiString;
var
  i: Integer;
begin
...
procedure TMESParser.Init;
begin
  OutputStream.Clear;

end;

procedure TMESParser.Final;
begin
...
begin
  if zstr = '' then
  begin
    Consoledebug('ERROR! Got null string when expecting a curly bracket pair {}');
    Result := '';
    exit;
...
(* End of Arbitrary Code *)
 ...
 { --------------------------------------------------------------------------- }
 { ---- implementation for TMESParserScanner ---- }

procedure TMESParserScanner.NextCh;
...
function TMESParserScanner.Comment: Boolean;
var
  level: Integer;
...
procedure TMESParserScanner.Get(var sym: Integer);
var
  breakcnt: Integer;
  state: Integer;
begin   {Get}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if breakcnt > 1 then
        begin
          if parse_state <> IDENTIFIER_SKIP then
  ...
  begin
    ...
    case state of
      ...
      8: if ((CurrInputCh >= CHR(9)) and (CurrInputCh <= CHR(10)) or (CurrInputCh = CHR(13)) or (CurrInputCh >= ' ') and
          (CurrInputCh <= '"') or (CurrInputCh >= '$') and (CurrInputCh <= 'z') or (CurrInputCh = '|') or (CurrInputCh >= '~')) then
        begin
      ...
      20: if ((CurrInputCh >= CHR(9)) and (CurrInputCh <= CHR(10)) or (CurrInputCh = CHR(13)) or (CurrInputCh >= ' ') and
          (CurrInputCh <= '"') or (CurrInputCh >= '$') and (CurrInputCh <= 'z') or (CurrInputCh = '|') or (CurrInputCh >= '~')) then
        begin
          state := 8;
        end
...
constructor TMESParserScanner.Create;
begin
  inherited;
...
destructor TMESParserScanner.Destroy;
begin
  fHashList.Free;
  fHashList := nil;
 ...
 { --------------------------------------------------------------------------- }
 { ---- implementation for TMESParser ---- }

constructor TMESParser.Create(AOwner: TComponent);
begin
  inherited;
  Scanner := TMESParserScanner.Create;
...
function TMESParser.ErrorStr(const ErrorCode: Integer; const Data: ansiString): ansiString;
begin
  case ErrorCode of
...
function TMESParser.GetScanner: TMESParserScanner;
begin
  Result := Scanner as TMESParserScanner;
end; {GetScanner}

function TMESParser._In(var s: SymbolSet; x: Integer): Boolean;
begin
  _In := x mod setsize in s[x div setsize];
end;  {_In}

procedure TMESParser._MESParser;
...
procedure TMESParser._MESEntry;
var
  i: Integer;
begin
  ...
  begin
    parse_state := IDENTIFIER_SKIP;
    Get;
    consoledebug('Junk at the beginning of line' + lexstring + '!', False);
  end;
  parse_state := PARSE_STATE_NORMAL;
  *)
  ...
  currentmessagefile.entries[CurrentMessageFile.entrycnt].add_linebreak := False;
  ...
  while fCurrentInputSymbol = numberSym do
  begin
    parse_state := IDENTIFIER_SKIP;
    Get;
    consoledebug('Skipping integer data outside brackets: "' + lexstring + '" at line ' + IntToStr(
      CurrentMessageFile.entries[CurrentMessageFile.entrycnt].index), False);
  end;
  ...
  begin
    parse_state := IDENTIFIER_SKIP;
    Get;
    consoledebug('Skipping string data outside brackets: "' + lexstring + '" at line ' + IntToStr(
      CurrentMessageFile.entries[CurrentMessageFile.entrycnt].index), False);
  end;
...
function TMESParser.GetMajorVersion: Integer;
begin
  Result := 0;
end;

function TMESParser.GetMinorVersion: Integer;
begin
  Result := 0;
end;

function TMESParser.GetRelease: Integer;
begin
  Result := 0;
end;

function TMESParser.GetBuild: Integer;
begin
  Result := 0;
end;

function TMESParser.GetBuildDate: TDateTime;
const
  BDate = 39638;
  Hour = 23;
  Min = 03;
...
function TMESParser.GetVersion: AnsiString;
var
  MinorVersionStr: AnsiString;
begin
...
function TMESParser.GetVersionComment: AnsiString;
begin
  Result := 'Arcanum MES Parser';
end;

function TMESParser.GetGrammarAuthor: AnsiString;
begin
  Result := 'T. Pitkänen';
end;

function TMESParser.GetGrammarCopyright: AnsiString;
begin
  Result := '(c) 2008 T. Pitkänen';
end;

procedure TMESParser.SetVersion(const Value: AnsiString);
begin
  // This is a read only property. However, we want the value
  // to appear in the Object Inspector during design time.
end;

procedure TMESParser.Parse;
begin
  errDist := minErrDist;
  GetScanner._Reset;
  Get;
...
procedure TMESParser.InitSymSet;
begin
  symSet[0, 0] := [EOFSYMB];
end; {InitSymSet}
