...
uses
  Windows, ModuleLoader, mesfileio, Messages, SysUtils, Variants, Classes,
...
type
  TForm9 = class(TForm)
    ...
    procedure pcnormaltextKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure pcdumbtextKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses MESParser, OllamaGenericDialog;
...
begin
  ...
  Form9.ListBox1.Items[ind] := IntToStr(GameRD_npc_m2m.entries[ind].
    index div 20) + ' [' + snippet + ']';
end;
...
begin
  ...
  begin
    ...
    Form9.ListBox1.Items.Add(IntToStr(GameRD_npc_m2m.entries[t].index div 20) +
      ' [' + snippet + ']');
  end;
end;
...
begin
  ...
  begin
    ...
    smartdataind := getmesindexbyid(GameRD_npc_m2m.entries[selindex].index,
      GameRD_npc_m2m);
    dumbdataind := getmesindexbyid(GameRD_npc_m2m.entries[selindex].index,
      GameRD_npc_m2m_dumb);
    ...
    entryidScript.Caption := IntToStr(GameRD_npc_m2m.entries[selindex].
      index div 20);
...
procedure TForm9.pcnormaltextKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TForm9.pcdumbtextKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
form30.showmodal;
 ...
 begin
  pcnormaltext.Text := form30.ollamaresult.text;
   GameRD_npc_m2m.entries[smartdataind].messagestr := pcnormaltext.Text;
     UpdateJournalListItem(ListBox1.ItemIndex);
 end;
end;
...
begin
form30.ollamaprompt.Text := 'Generate a low intelligence version of the following text: '+#13#10#13#10+pcnormaltext.Text;
form30.showmodal;
    if form30.modalresult = mrOk then
 begin
  pcdumbtext.Text := form30.ollamaresult.text;
   GameRD_npc_m2m_dumb.entries[smartdataind].messagestr := pcdumbtext.Text;
     UpdateJournalListItem(ListBox1.ItemIndex);
 end;
end;
