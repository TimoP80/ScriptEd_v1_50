...
procedure InsertPlayerOption(Text: String; nodeindex: Integer;
  ins_index: Integer);
...
begin
  ...
  begin
    CurDLG.nodes[nodeind].PlayerOptioncnt := CurDLG.nodes[nodeind]
      .PlayerOptioncnt - 1;
    exit;
  end;
  ...
  begin
    CurDLG.nodes[nodeind].playeroptions[t] := CurDLG.nodes[nodeind]
      .playeroptions[t + 1];
  end;
  CurDLG.nodes[nodeind].PlayerOptioncnt := CurDLG.nodes[nodeind]
    .PlayerOptioncnt - 1;
...
var
  ...
  t: Integer;
begin
  setlength(CurDLG.nodes[nodeindex].playeroptions,
    CurDLG.nodes[nodeindex].PlayerOptioncnt + 1);
  new(CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex]
    .PlayerOptioncnt]);
  for t := CurDLG.nodes[nodeindex].PlayerOptioncnt downto ins_index + 1 do
  begin
    CurDLG.nodes[nodeindex]^.playeroptions[t]^ := CurDLG.nodes[nodeindex]
      .playeroptions[t - 1]^;
  end;
  setlength(CurDLG.nodes[nodeindex].playeroptions,
    CurDLG.nodes[nodeindex].PlayerOptioncnt + 1);
...
begin
  ...
  new(CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex]
    .PlayerOptioncnt]);
  CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex].PlayerOptioncnt]
    .lineindex := 0;
  CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex].PlayerOptioncnt]
    .Text := Text;
  CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex].PlayerOptioncnt]
    .gendertest := NO_GENDER;
  CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex].PlayerOptioncnt]
    .iqtest := 5;
  CurDLG.nodes[nodeindex].playeroptions[CurDLG.nodes[nodeindex].PlayerOptioncnt]
    .conditions := '';
...
procedure LoadDialogueToPointer(dlgptr: PDialogueFile; filename: String);
var
  t: Integer;
begin

  if (dlgptr.nodecount > 0) then
  begin
    for t := 0 to dlgptr.nodecount - 1 do
    begin
      if dlgptr.nodes[t] <> nil then
      begin
        dispose(dlgptr.nodes[t]);
      end;
    end;
  end;
  ...
  begin
    Inc(dlgptr.nodecount);
    if isconsole = False then
      ConsoleDebug('Successfully loaded dialogue file "' + filename + '" with '
        + IntToStr(dlgptr.nodecount) + ' nodes.', False);
    dlgptr.dlgfilename := extractfilename(filename);
  end
  else
  begin
    ConsoleDebug('There was an error parsing the dialogue file!');
    DLGParser.ListStream.SaveToFile
      ('ERRORLOG_' + changefileext
      (extractfilename(DLGParser.SourceFileName), '.lst'));
    if isconsole = False then
      MessageDlg(format('An error has occured parsing the dialogue file %s',
...
begin
  ...
  begin
    ...
    if isconsole = False then
      ConsoleDebug('Successfully loaded dialogue file "' + filename + '" with '
        + IntToStr(CurDLG.nodecount) + ' nodes.', False);
